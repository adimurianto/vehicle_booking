$(document).ready(function(){
    ...
    var save_method = "add";
      ...
      $(document).on("click",".btn-update-menu-trigger",function(e){
          e.preventDefault();
          var _ = $(this);
          var id = _.attr('data-id');
          toggleLoader($("#crud-loader-menu"),"hide");
          openPopupMenuAccess(id);
      });
      ...
      function openPopupMenuAccess(id){
        $.ajax({
          url: BaseUrl+"Group/getbyid",
          method: "POST",
          data: {id:id},
          ...
          success: function(data){
            if(data != null){
              $('[name="authority"]').val(data.id);
              $('[name="name"]').val(data.name);
              $('.popup-menu-title').text('Atur Akses Menu '+data.name);
              getMenuAccess(id);
            }else{
              codehubalert("Data tidak ditemukan","error");
            }
    ...
    function getMenuAccess(id){
      $.ajax({
        ...
        success: function(res){
          $('#menu-access').html(res);
          $("#modal-menu-access").modal('show');
        },
        error: function(jqXHR, textStatus, errorThrown){
          codehubalert('Terjadi kesalahan pada server','error');
        }
    ...
    function updateMenuAccess(){
      $.ajax({
        url: BaseUrl+"Menu_access/updateMenuAccess",
        type: "POST",
        dataType: "JSON",
        data: $('#form-menu').serialize(),
        beforeSend: function(){
          toggleLoader($("#crud-loader-menu"),"show");
        },
        success: function(data){
          if(data.status){
            codehubalert(data.msg,'success');
            $("#modal-menu-access").modal('hide');
          }else{
            codehubalert(data.msg,'error');
          }
          toggleLoader($("#crud-loader-menu"),"hide");
      ...
      $('#form').validate({
          ...
          submitHandler: function(e){
            var url;
            if(save_method == "add"){
              url = BaseUrl+"Group/addData";
            }else{
              url = BaseUrl+"Group/updateData";
            }
