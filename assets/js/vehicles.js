$(document).ready(function(){
  ...
  var save_method = "add";
    ...
    function deletedData(){
      $.ajax({
        url: BaseUrl+"Vehicles/deleteData/",
        method: "POST",
        ...
        $.ajax({
          ...
          success: function(data){
            ...
            if(data != null){
              ...
              $('select[name="ownership"] option[value="'+data.ownership+'"').prop('selected',true);  
              $('select[name="location_id"] option[value="'+data.location_id+'"').prop('selected',true);  
              $('select[name="status"] option[value="'+data.status+'"').prop('selected',true);

              if(data.plate_number.length > 0){
                $('input[name="plate_number"]').parent().addClass("active");  
              }
    ...
    $('#form').validate({
        ...
        submitHandler: function(e){
          var url;
          if(save_method == "add"){
            url = BaseUrl+"Vehicles/addData";
          }else{
            url = BaseUrl+"Vehicles/updateData";
          }
